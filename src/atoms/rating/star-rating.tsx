...
export const StarRating: React.FC<{ rating: string; className?: string }> = ({ rating, className }) => {
  const stars = [];
  for (let i = 0; i < 5; i++) {
    stars.push(<img className="w-5" key={i} src={i < parseInt(rating) ? activeStar : inactiveStar} />);
  }

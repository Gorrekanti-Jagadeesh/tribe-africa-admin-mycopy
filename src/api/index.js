...
//   }
// };
export const fetchImageByEntryId = async (entryId) => {
...
export const fetchCurrencies = async () => {
  ...
  try {
    ...
    if (res?.data) {
      return res?.data.supported_codes.map((code) => {
        return {
          currencyCode: code[0],
          currencyName: code[1],
        };
...
export const getDataByEntryType = async (entryType, key, format) => {
...
// };
export const getDataByDocumentType = (entryType, fields, language = 'en') => {
  const fieldsQuery = fields ? fields.join(', ') : '*';
  return sanityClient.fetch(`*[_type == "${entryType}" && language == "${language}"]{${fieldsQuery}}`);
};
export const getDataByDocumentTypeWithId = (entryType, fieldType, id, fields) => {
  const fieldsQuery = fields ? fields.join(', ') : '*';
...
export const getEntryDataById = (id) => {
  return sanityClient.fetch(`*[_id == '${id}'][0]`);
};
export const getHotelsInLocationWithLimit = (countryId) => {
  sanityClient
...
export const sanityImageUrlBuilder = (image) => {

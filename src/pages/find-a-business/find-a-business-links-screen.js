...
const FindABusinessLinksScreen = ({ country }) => {
  const navigation = useNavigate();
  return _jsxs('div', {
    className: 'p-2 md:p-4 m-auto',
    children: [
      _jsxs('div', {
        className: 'flex mb-4',
        children: [
          _jsxs('h1', { className: 'text-4xl font-bold', children: ['Find A Business in ', fromKebabCase(country)] }),
          _jsx(Button, { className: 'ms-auto', children: 'List your Business' }),
        ],
      }),
      _jsx('div', {
        className: 'flex',
        children: _jsx('div', {
          className: 'md:columns-5',
          children: data.map((category, index) =>
            _jsxs(
              'div',
              {
                className: 'mb-3',
                children: [
                  _jsx('h1', { className: 'text-xl font-bold w-3/4 cursor-pointer', children: category.mainCat }),
                  category.hasSubCategory
                    ? _jsx('ul', {
                        children: category.subItems.map((sub, subIndex) =>
                          _jsx(
                            'li',
                            {
                              ...
                              onClick: () => {
                                if (category.mainCat === 'Accommodation') {
                                  navigation(
                                    `/${toKebabCase(country)}/business/${toKebabCase('Accommodation')}/${sub.id}`
                                  );
                              ...
                              children: typeof sub === 'object' ? sub.name : sub,
                            },
                            subIndex
                          )
                        ),

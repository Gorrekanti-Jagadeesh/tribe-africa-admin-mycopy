...
const AccommodationDetailsContainer = () => {
  ...
  const { categoryInfoId } = useParams();
  const fetchReviewsByHotelId = async (id) => {
  ...
  if (error) return _jsxs('div', { children: ['Error fetching reviews: ', error.message] });
  if (!data) return _jsx(_Fragment, { children: 'data not received yet' });
  return _jsx('div', {
    children: _jsx(AccomodationDetailsScreen, {
      hostel: data,
      reviews: reviews,
